    ...
    if (finalCount <= 3) {
      ...
      for (const member of members.rows) {
        const notif = await db.query(
          ...
          [family_id, member.username, `${trimmedName} was added with low stock (${finalCount} left)`, 'stock']
        );
...
router.post('/consume/:id', async (req, res) => {
  ...
  try {
    ...
    for (const member of members.rows) {
      const notif = await db.query(
        'INSERT INTO notifications (family_id, username, message, type) VALUES ($1, $2, $3, $4) RETURNING *',
        [family_id, member.username, `${username} consumed ${product.name} — ${product.count} left`, 'consume']
      );
      io.to(`user_${member.username}`).emit('new_notification', notif.rows[0]);
    }
    ...
    if (product.count <= 3) {
      ...
      for (const member of allMembers.rows) {
        const notif = await db.query(
          'INSERT INTO notifications (family_id, username, message, type) VALUES ($1, $2, $3, $4) RETURNING *',
          [family_id, member.username, `${product.name} is running low — only ${product.count} left`, 'stock']
        );
        io.to(`user_${member.username}`).emit('new_notification', notif.rows[0]);
      }
    }
    ...
    if (product.count === 0) {
      ...
      for (const member of allMembers.rows) {
        const notif = await db.query(
          'INSERT INTO notifications (family_id, username, message, type) VALUES ($1, $2, $3, $4) RETURNING *',
          [family_id, member.username, `${product.name} is now out of stock!`, 'stock']
        );
        io.to(`user_${member.username}`).emit('new_notification', notif.rows[0]);
      }
    }
