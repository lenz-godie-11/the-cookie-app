...
import { User, Lock, Bell, Palette, LogOut, Trash2, UserMinus, Eye, EyeOff, CheckCircle, X } from 'lucide-react';
import axios from 'axios';

const API_BASE_URL = (import.meta.env.VITE_API_URL || 'https://the-cookie-app.onrender.com') + '/api';

export default function Settings({ setUsername, setFamilyId }) {
  ...
  const handleNotifToggle = (type) => {
    ...
    showToast(`${type} notifications ${updated[type] ? 'enabled' : 'disabled'}`);
  };

  const handleLeaveFamily = async () => {
    try {
      await axios.post(`${API_BASE_URL}/auth/leave-family`, { username, family_id });
      localStorage.clear();
      setUsername('');
      setFamilyId('');
      navigate('/login');
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to leave family', 'error');
    }
  };
  ...
  const handleDeleteFamily = async () => {
    try {
      ...
      showToast(err.response?.data?.message || 'Failed to delete family', 'error');
    }
  };
      ...
      <div className="flex gap-6">
          ...
          <div className="bg-[#121214] border border-white/5 rounded-2xl p-2 space-y-1">
            ...
            <div className="border-t border-white/5 mt-2 pt-2">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-red-400 hover:text-red-300 hover:bg-[#1a1a1c] transition-all"
              >
        ...
        <div className="flex-1">
          ...
          {activeSection === 'account' && (
            <div className="space-y-4">
              <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
                ...
                <p className="text-slate-500 text-xs mb-4">Current username: <span className="text-slate-300 font-bold">{username}</span></p>
                <input
                  type="text"
                  placeholder="New username"
                  value={accountForm.newUsername}
                  onChange={(e) => setAccountForm(prev => ({ ...prev, newUsername: e.target.value }))}
                  className="w-full bg-[#1a1a1c] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:ring-1 focus:ring-[#3b5d8f] text-sm mb-3"
                />
                <button
                ...
                <div className="space-y-3">
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-[#1a1a1c] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:ring-1 focus:ring-[#3b5d8f] text-sm pr-12"
                    />
          ...
          {activeSection === 'notifications' && (
            <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
              <h2 className="text-white font-bold mb-4 flex items-center gap-2">
                <Bell size={16} className="text-[#3b5d8f]" /> Notification Preferences
              </h2>
              <div className="space-y-4">
                {[
                  { key: 'consume', label: 'Item consumed', desc: 'When a family member consumes an item' },
                  { key: 'stock', label: 'Low stock alerts', desc: 'When an item is running low or out of stock' },
                  { key: 'member', label: 'New member', desc: 'When someone joins your family' },
                  { key: 'message', label: 'New messages', desc: 'When you receive a private or group message' },
                ].map(({ key, label, desc }) => (
                  <div key={key} className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                    ...
                    <button
                      ...
                      className={`w-12 h-6 rounded-full transition-all relative ${
                        notifSettings[key] ? 'bg-[#3b5d8f]' : 'bg-[#1a1a1c] border border-white/10'
                      }`}
                    >
            ...
            <div className="space-y-4">
              {!isAdmin && (
                <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
                  <h2 className="text-white font-bold mb-2 flex items-center gap-2">
                    <UserMinus size={16} className="text-yellow-400" /> Leave Family
                  </h2>
                  <p className="text-slate-400 text-sm mb-4">You will lose access to all family data and need a new invite link to rejoin.</p>
                  {!confirmLeave ? (
                    <button
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="bg-[#1a1a1c] border border-white/10 text-slate-300 px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#252528] transition-all"
                      >
                        Cancel
                      </button>
                    </div>
              ...
              {isAdmin && (
                <div className="bg-[#121214] border border-red-500/20 rounded-2xl p-6">
                  ...
                  <p className="text-slate-400 text-sm mb-4">This will permanently delete the family and all its data including products, messages and members. This cannot be undone.</p>
                  {!confirmDelete ? (
                    <button
